...
type
  TCompWriterJS = class;
  TCWPFindAncestorEvent = procedure(Writer: TCompWriterJS; Component:
TComponent;
    const Name: string; var Ancestor, RootAncestor: TComponent) of object;
  TCWPGetMethodName = procedure(Writer: TCompWriterJS; Instance:
TPersistent;
    PropInfo: PPropInfo; out Name: String) of object;
  TCWPGetParentPropertyEvent = procedure(Writer: TCompWriterJS;
    Component: TComponent; var PropName: string) of object;
  TCWPDefinePropertiesEvent = procedure(Writer: TCompWriterJS;
...
FOnDefineProperties write FOnDefineProperties;
  public
    ...
    property AccessClass: string read FAccessClass write FAccessClass; // classname used to access protected TComponent members like SetChildOrder
...
type
  TCWPDefinePropertiesProc = procedure(Sender: TCompWriterJS;
    Instance: TPersistent; const Identifier: string; var Handled: boolean);
...
procedure UnregisterDefinePropertiesPas(
  const OnDefineProperties: TCWPDefinePropertiesProc);
procedure CallDefinePropertiesPas(Writer: TCompWriterJS; Instance:
TPersistent;
...
begin
  Writer:=TCompWriterJS.Create(AStream);
  try
...
procedure CallDefinePropertiesPas(Writer: TCompWriterJS;
  Instance: TPersistent; const Identifier: string; var Handled: boolean);
var
  i: Integer;
  E: TDefinePropertiesPas;
...
procedure TCompWriterJS.AddToAncestorList(Component: TComponent);
begin
 
FAncestors.AddObject(Component.Name,TPosComponent.Create(FAncestors.Count,Component));
end;

procedure TCompWriterJS.DetermineAncestor(Component: TComponent);
var
  i : Integer;
  C: TComponent;
begin
...
procedure TCompWriterJS.SetNeededUnits(const AValue: TStrings);
begin
  if FNeededUnits=AValue then Exit;
  FNeededUnits.Assign(AValue);
end;

procedure TCompWriterJS.SetRoot(const AValue: TComponent);
begin
  FRoot:=AValue;
  FLookupRoot:=FRoot;
end;

procedure TCompWriterJS.WriteComponentData(Instance: TComponent);
var
  HasAncestor: Boolean;
...
procedure TCompWriterJS.WriteChildren(Component: TComponent;
...
procedure TCompWriterJS.WriteProperty(Instance: TPersistent;
  PropInfo: PPropInfo);
type
  TSet = set of 0..31;
var
...
WriteAssign(PropName,GetBoolLiteral(LongBool(Int32Value)));
    ...
    tkMethod:
      begin
        ...
        //debugln(['TCompWriterJS.WriteProperty ',dbgs(MethodValue.Data),' ',dbgs(MethodValue.Code),' ',dbgs(DefMethodValue.Data),' ',dbgs(DefMethodValue.Code)]);
...
WriteAssign(PropName,GetStringLiteral(AnsiString(VarValue.vstring)));
            ...
            //varrecord : (vrecord : pointer;precinfo : pointer);
              ...
              {$IFDEF VerboseCompWriterPas}
              debugln(['TCompWriterJS.WriteProperty Property="',PropName,'"
...
then
            begin
              ...
              begin
                // value is a component, and it is the same as in the ancestor
                // Note: a descendant has new instances with same names AncestorComponent := Component;
              end;
            end;
          end;
...
procedure TCompWriterJS.WriteProperties(Instance: TPersistent);
var
  PropCount, i: integer;
  PropList: PPropList;
begin
...
procedure TCompWriterJS.WriteDefineProperties(Instance: TPersistent);
var
  Col: Integer;
...
procedure TCompWriterJS.WriteCollection(PropName: string;
...
function TCompWriterJS.GetComponentPath(Component: TComponent): string;
var
  Name: String;
  C: TComponent;
begin
...
function TCompWriterJS.GetBoolLiteral(b: boolean): string;
begin
  if b then
    Result:='True'
  else
...
function TCompWriterJS.GetCharLiteral(c: integer): string;
begin
  case c of
  32..126: Result:=''''+chr(c)+'''';
...
function TCompWriterJS.GetWideCharLiteral(c: integer): string;
begin
  case c of
  32..126:
    Result:=''''+Chr(c)+'''';
...
function TCompWriterJS.GetStringLiteral(const s: string): string;
...
function TCompWriterJS.GetWStringLiteral(p: PWideChar; Count: integer):
string;
...
function TCompWriterJS.GetFloatLiteral(const e: Extended): string;
var
  s: String;
begin
  s:='';
...
function TCompWriterJS.GetCurrencyLiteral(const c: currency): string;
var
  i: int64 absolute c;
var
  s: String;
...
function TCompWriterJS.ShortenFloat(s: string): string;
var
  p, i: SizeInt;
begin
...
function TCompWriterJS.GetEnumExpr(TypeInfo: PTypeInfo; Value: integer;
  AllowOutOfRange: boolean): string;
var
  PT: PTypeData;
begin
...
function TCompWriterJS.GetVersionStatement: string;
begin
  Result:='// Format version '+IntToStr(CSPVersion);
end;

constructor TCompWriterJS.Create(AStream: TStream);
var
  C: TAccessComp;
begin
  FIndentStep:=2;
...
destructor TCompWriterJS.Destroy;
begin
  FreeAndNil(FNeededUnits);
  inherited Destroy;
end;

procedure TCompWriterJS.WriteComponentCreate(Component: TComponent);
var
...
procedure TCompWriterJS.WriteComponent(Component: TComponent);
var
  OldAncestor : TPersistent;
  OldRoot, OldRootAncestor : TComponent;
  i: integer;
...
procedure TCompWriterJS.WriteDescendant(ARoot: TComponent; AAncestor:
TComponent);
var
  i: integer;
begin
...
procedure TCompWriterJS.WriteIndent;
begin
  Write(StringOfChar(' ',CurIndent));
end;

procedure TCompWriterJS.Write(const s: string);
begin
  if s='' then exit;
  FStream.Write(s[1],length(s));
end;

procedure TCompWriterJS.WriteLn;
begin
  Write(LineEnding);
end;

procedure TCompWriterJS.WriteStatement(const s: string);
begin
  WriteIndent;
  Write(s);
  WriteLn;
...
procedure TCompWriterJS.WriteAssign(const LHS, RHS: string);
begin
  WriteIndent;
  Write(LHS);
  Write(AssignOp);
...
procedure TCompWriterJS.WriteWithDo(const Expr: string);
begin
...
procedure TCompWriterJS.WriteWithEnd;
begin
  Unindent;
  if not (cwpoNoWithBlocks in Options) then
    WriteStatement('end;');
...
function TCompWriterJS.CreatedByAncestor(Component: TComponent): boolean;
begin
...
procedure TCompWriterJS.AddNeededUnit(const AnUnitName: string);
begin
  if FNeededUnits.IndexOf(AnUnitName)>=0 then exit;
  FNeededUnits.Add(AnUnitName);
end;

procedure TCompWriterJS.Indent;
begin
  CurIndent:=CurIndent+IndentStep;
end;

procedure TCompWriterJS.Unindent;
begin
  CurIndent:=CurIndent-IndentStep;
end;
